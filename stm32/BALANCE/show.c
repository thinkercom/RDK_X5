...
void show_task(void *pvParameters)
{
   ...
   {	
        ...
        if(10<=Voltage && Voltage<10.5f && LowVoltage_1<2)LowVoltage_1++; //10.5V, first buzzer when low battery //10.5V，低电量时蜂鸣器第一次报警
...
{  
	...
	if((Servo_init_angle_adjust == 0 && show_imu == 0 && show_servo == 0) ||  motor_test == 1)//正常控制
   {
	 ...
	 {	
		 ...
		 else if(Car_Mode==Akm_Car||Car_Mode==Diff_Car||Car_Mode==FourWheel_Car || Car_Mode==Tank_Car)
		 {
			 ...
			 else                      OLED_ShowString(90,0,"+"),OLED_ShowNumber(100,0, Deviation_gyro[2],4,12);	//陀螺仪z轴零点漂移数据	
		 ...
		 else if(Car_Mode==Akm_Car||Car_Mode==Diff_Car||Car_Mode==Tank_Car)
		 {
			 ...
			 else            OLED_ShowString(60,10,"+"),  OLED_ShowNumber(75,10, gyro[2],5,12);		//z轴陀螺仪数据	
		 }
